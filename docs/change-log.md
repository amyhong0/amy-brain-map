...
| `f6b4b39` | feat: A2A 방식 텍스트+이미지 병렬 분석 (텍스트 LLM 유지, Vision 모델 추가) |
...
## 16. 인스타그램 캐러셀 이미지 분석 및 제목 생성 개선 (`app/api/knowledge/route.ts`)

### 문제
- 인스타그램 캐러셀 게시글에서 이미지 파싱이 제대로 동작하지 않았음
- 제목에 "인스타그램 게시글" 등 일반적인 제목이 사용되어 다른 게시글과 구분되지 않음
- 본문에 이미지 분석 내용이 포함되지 않아 지식으로서의 가치가 낮음

### 수정
1. **(커밋 `a46f488`)** 인스타그램 URL 처리 시 early return 제거하여 A2A 이미지 분석이 실행되도록 개선
2. `fetchInstagramContent()`에서 캐러셀 이미지 URL 수집 강화:
   - `og:image` 메타 태그에서 첫 번째 이미지 수집
   - `img` 태그에서 `cdninstagram.com`/`cdninstagram` 패턴의 추가 이미지 수집
   - 중복 URL 제거 로직 개선
3. 첫 번째 이미지 분석으로 제목 후보 추출:
   - Vision 모델에 첫 번째 이미지 전송해 제목/핵심 문구 추출
   - 추출된 제목이 "none"이거나 실패하면 `visionTitleCandidate`로 사용
4. 이미지 설명과 텍스트 본문을 결합하여 LLM에 전달:
   - `[이미지 내용]` 섹션으로 이미지 설명 추가
   - LLM이 텍스트와 이미지를 함께 분석하여 더 풍부한 content 생성
5. 제목 생성 시 LLM이 일반적인 제목을 반환하는 경우 방지:
   - `GENERIC_TITLES` Set으로 "인스타그램 게시글", "웹 문서" 등 필터링
   - Fallback: 첫 번째 keyword 또는 content 첫 줄 사용
6. systemPrompt에 일반적인 제목 사용 금지 지시 추가
7. 인스타그램 게시글의 경우, 이미지에 제목이 있을 경우 우선 사용하고 없다면 본문과 이미지 내용을 바탕으로 적합한 제목 생성하도록 LLM에 지시

---

## 15. 블로그/인스타그램 제목 개선 및 A2A UI 반영 (`app/api/knowledge/route.ts`, `app/page.tsx`)

### 문제
- 블로그 주소 추가 시 제목에 본문 전체가 들어가는 문제
- 인스타그램 캐러셀 게시글에서 첫 번째 이미지에 제목이 있는 경우가 많지만 활용되지 않음
- 스펠 로그/에이전트 말풍선에 A2A 병렬 처리 의미가 반영되지 않음

### 수정
1. **(커밋 `db8697a`)** (`app/api/knowledge/route.ts`) 제목 선택 로직 개선:
   - `og:title`, `h1`, `<title>` 중 120자 미만인 첫 번째 유효 제목 사용
   - 120자 초과이면 URL을 제목으로 사용해 본문이 제목으로 들어가는 문제 방지
2. **(커밋 `db8697a`)** (`app/api/knowledge/route.ts`) 첫 이미지 제목 후보 추출:
   - Vision 모델에 첫 번째 이미지만 전송해 제목/핵심 문구 추출
   - LLM 결과와 비교해 제목이 없거나 너무 길 경우 `visionTitleCandidate`로 대체
3. **(커밋 `db8697a`)** (`app/page.tsx`) 스펠 로그/말풍선 업데이트:
   - `runAgentWorkflow` 단계를 A2A 병렬 처리에 맞게 변경
   - "A2A 협력 프로토콜", "A2A 텍스트 분석", "A2A 비전 분석", "A2A 병렬 결과 취합" 등으로 표현
   - 지식 추가 성공 메시지에 "(A2A)" 표시
