...
import {
  Activity, Bot, BrainCircuit, Check, ChevronDown, ChevronUp, Copy, ExternalLink,
  ListChecks, LogIn, LogOut, Network, Play, RefreshCw, Search, Send, ShieldCheck, Sparkles, UserRound, X,
} from 'lucide-react';
import UnconsciousMap from '@/components/unconscious/UnconsciousMap';
import ReviewQueue, { candidateReviewCopy } from '@/components/unconscious/ReviewQueue';
...
export default function Home() {
  ...
  const updateCandidate = async (candidate: DiscoveryCandidate, status: 'approved' | 'rejected' | 'pending') => {
    ...
    try {
      ...
      if (status === 'approved') setApprovalNotice(nextPending ? '이 연결 검토 대상을 지도에 반영했습니다. 다음 검토 대상이 열렸습니다.' : '이 연결 검토 대상을 내 지도에 반영했습니다.');
      if (status === 'rejected') setApprovalNotice(nextPending ? '이 연결 검토 대상을 제외했습니다. 다음 검토 대상이 열렸습니다.' : '이 연결 검토 대상을 제외했습니다. 원래 방문 기록은 그대로 유지됩니다.');
    } catch (updateError) {
      setError(visitorFacingError(updateError, '연결 상태를 변경하지 못했습니다.'));
    }
  };

  const reviewCandidate = (candidate: DiscoveryCandidate) => {
    setSelectedCandidate(candidate);
    setIsReviewListOpen(false);
    window.setTimeout(() => document.getElementById('connection-review')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 0);
  };
  ...
  return (
    <main className="brain-canvas min-h-screen text-slate-900">
      ...
      <div className="relative mx-auto max-w-[1680px] px-4 pb-10 md:px-8 lg:px-10">
        ...
        {user && <section className="brain-card mt-5 grid gap-4 rounded-3xl p-5 md:p-6 lg:grid-cols-[1fr_auto]"><div><p className="text-base font-bold text-slate-900">{user.name || user.email}님의 개인 지도</p></div><div className="flex flex-wrap items-center gap-2"><StatusPill tone="green"><ShieldCheck className="h-3.5 w-3.5" />계정 보안</StatusPill><button type="button" onClick={exportMyData} disabled={isExporting} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl border border-blue-200 bg-white px-3 text-xs font-bold text-blue-700 transition hover:bg-blue-50 disabled:cursor-not-allowed disabled:opacity-55">{isExporting ? <RefreshCw className="h-3.5 w-3.5 animate-spin" /> : <Copy className="h-3.5 w-3.5" />}{isExporting ? '내보내기 준비 중' : '내 기록 내보내기'}</button></div></section>}
        ...
        <section className="mt-5 grid gap-5 lg:grid-cols-[1.15fr_.85fr]"><section className="brain-card rounded-3xl p-5 md:p-6"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-blue-600">Memory traces</p><h2 className="mt-1 text-lg font-extrabold text-slate-950">{queryResult ? '이번 질문에 반응한 흔적' : '최근 동기화된 탐색 흔적'}</h2></div>{queryResult && <StatusPill tone="violet">지도 강조 중</StatusPill>}</div><div className="mt-4 grid gap-3 sm:grid-cols-2">{visibleVisits.slice(0, 8).map((visit) => { const score = typeof (visit as unknown as { score?: unknown }).score === 'number' ? (visit as unknown as { score: number }).score : null; return <article key={visit.id} className="brain-card-interactive rounded-2xl border border-slate-100 bg-slate-50/70 p-3.5"><div className="flex items-start justify-between gap-2"><p className="line-clamp-2 text-sm font-bold leading-5 text-slate-800">{visit.title || visit.domain}</p>{score !== null && <span className="shrink-0 rounded-full bg-violet-100 px-2 py-1 text-[10px] font-bold text-violet-700">{Math.round(score * 100)}%</span>}</div><p className="mt-2 truncate text-xs font-semibold text-blue-600">{visit.domain}</p><p className="mt-2 text-[11px] text-slate-500">{formatDate(visit.lastVisitTime)} · {visit.visitCount}회 방문</p></article>; })}{!visibleVisits.length && <p className="col-span-2 rounded-2xl border border-dashed border-slate-200 px-4 py-10 text-center text-sm text-slate-500">{user ? '아직 동기화된 탐색 흔적이 없습니다. Chrome 확장 프로그램을 연결해 보세요.' : 'Google 로그인 뒤 나만의 탐색 지도를 시작할 수 있습니다.'}</p>}</div></section>
          <section id="connection-review" className="brain-card scroll-mt-24 rounded-3xl p-5 md:p-6"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-violet-600">Review a suggestion</p><h2 className="mt-1 text-lg font-extrabold text-slate-950">이 제안은 무엇을 뜻하나요?</h2></div><button type="button" onClick={() => setIsReviewListOpen((open) => !open)} aria-label={isReviewListOpen ? '전체 연결 검토 대상 접기' : '전체 연결 검토 대상 펼치기'} aria-expanded={isReviewListOpen} aria-controls="all-review-candidates" className="grid min-h-11 min-w-11 place-items-center rounded-xl border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50 hover:text-slate-900">{isReviewListOpen ? <ChevronUp className="h-5 w-5" aria-hidden="true" /> : <ChevronDown className="h-5 w-5" aria-hidden="true" />}</button></div>{isReviewListOpen && <div id="all-review-candidates" className="mt-5 rounded-2xl border border-violet-100 bg-violet-50/60 p-3"><div className="flex items-center justify-between gap-3"><p className="text-xs font-extrabold text-violet-800">전체 연결 검토 대상</p><StatusPill tone="violet">{pendingCandidates.length}개</StatusPill></div>{pendingCandidates.length ? <div className="mt-3 max-h-72 space-y-2 overflow-y-auto pr-1">{pendingCandidates.map((candidate) => { const copy = candidateReviewCopy(candidate); return <button type="button" key={candidate.id} onClick={() => reviewCandidate(candidate)} className={`w-full rounded-xl border p-3 text-left transition ${candidate.id === selectedCandidate?.id ? 'border-violet-300 bg-white shadow-sm' : 'border-violet-100 bg-white/80 hover:border-violet-200 hover:bg-white'}`}><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-extrabold text-violet-800">{copy.category}</p><p className="mt-1 text-sm font-bold leading-5 text-slate-900">{copy.title}</p><p className="mt-1.5 text-xs leading-5 text-slate-600"><span className="font-bold text-slate-700">지도 반영:</span> {copy.mapEffect}</p></div><ChevronDown className="mt-1 h-4 w-4 shrink-0 -rotate-90 text-violet-500" aria-hidden="true" /></div></button>; })}</div> : <p className="mt-3 rounded-xl bg-white px-3 py-4 text-center text-sm text-slate-600">현재 승인 대기 중인 연결 검토 대상이 없습니다.</p>}</div>}{selectedCandidate ? <div className="mt-5">{(() => { const review = candidateReviewCopy(selectedCandidate); return <><div className="flex items-start justify-between gap-3"><p className="text-xs font-extrabold tracking-[0.12em] text-violet-700">{review.category}</p><StatusPill tone={selectedCandidate.status === 'approved' || selectedCandidate.status === 'auto_applied' ? 'green' : selectedCandidate.status === 'pending' ? 'amber' : 'slate'}>{selectedCandidate.status === 'approved' ? '지도에 반영됨' : selectedCandidate.status === 'pending' ? '승인 대기' : selectedCandidate.status === 'rejected' ? '제외됨' : '자동 반영됨'}</StatusPill></div><p className="mt-3 text-base font-extrabold leading-7 text-slate-900">{review.title}</p><div className="mt-4 rounded-2xl border border-violet-100 bg-violet-50/70 p-3.5"><p className="text-xs font-extrabold text-violet-800">승인하면 지도에는 이렇게 반영됩니다</p><p className="mt-1 text-sm leading-6 text-violet-950">{review.mapEffect}</p></div><div className="mt-4 rounded-2xl bg-slate-50 p-3.5"><p className="text-xs font-extrabold text-slate-700">이렇게 판단한 근거</p><p className="mt-1.5 text-sm leading-6 text-slate-600">{selectedCandidate.evidence.join(' ')}</p><p className="mt-2 text-xs leading-5 text-slate-500">기록된 도메인 · {selectedCandidate.sourceDomains.join(', ')}</p></div>{selectedCandidate.status === 'pending' && <div className="mt-5 flex flex-wrap gap-2"><button type="button" onClick={() => updateCandidate(selectedCandidate, 'approved')} className="inline-flex min-h-11 items-center gap-1.5 rounded-xl bg-emerald-600 px-3.5 text-xs font-bold text-white transition hover:bg-emerald-700"><Check className="h-3.5 w-3.5" aria-hidden="true" />승인하고 지도에 반영</button><button type="button" onClick={() => updateCandidate(selectedCandidate, 'rejected')} className="min-h-11 rounded-xl border border-slate-200 bg-white px-3.5 text-xs font-bold text-slate-600 transition hover:bg-slate-50">이 제안 제외</button></div>}</>; })()}</div> : <div className="mt-5 rounded-2xl bg-slate-50 px-4 py-10 text-center"><ListChecks className="mx-auto h-5 w-5 text-slate-400" aria-hidden="true" /><p className="mt-3 text-sm font-bold text-slate-700">연결 검토 대상을 선택하세요.</p><p className="mt-1 text-xs leading-5 text-slate-500">제안이 뜻하는 패턴과 승인했을 때의 결과를 이곳에서 확인할 수 있습니다.</p></div>}</section>
        </section>
