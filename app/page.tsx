...
export default function Home() {
  ...
  const removeNodeFromMap = async () => {
    ...
    const removal = pendingMapRemoval;
    const targets = removal.candidates.filter((candidate) => candidate.status !== 'rejected');
    if (targets.length === 0) {
    ...
    const previousCandidates = new Map(targets.map((candidate) => [candidate.id, candidate]));
    setIsRemovingFromMap(true);
    setError('');
    setApprovalNotice('');
    setCandidates((current) => current.map((candidate) => targetIds.has(candidate.id) ? { ...candidate, status: 'rejected' } : candidate));
    setPendingMapRemoval(null);
    clearMapHighlights();
    ...
    try {
      ...
      setCandidates((current) => current.map((candidate) => {
        if (byId.has(candidate.id)) return byId.get(candidate.id)!;
        if (failedIds.has(candidate.id)) return previousCandidates.get(candidate.id) || candidate;
        return candidate;
      }));

      if (failedIds.size > 0) {
        throw new Error(`${failedIds.size}개의 연결을 제거하지 못했습니다. 다시 시도해 주세요.`);
      }
      setApprovalNotice(`“${removal.label}”을(를) 지도에서 제거했습니다. 근거 방문 기록은 그대로 유지됩니다.`);
    } catch (removalError) {
  ...
  return (
    <main className="aether-dashboard brain-canvas min-h-screen text-slate-900">
      ...
      <div className="relative w-full px-4 pb-10 md:px-8 lg:px-10">
        ...
        {settingsOpen && <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-950/55 p-4 py-6 backdrop-blur-sm" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSettingsOpen(false); }}>
          <div role="dialog" aria-modal="true" aria-labelledby="history-settings-title" aria-describedby="history-settings-description" className="w-full max-w-4xl rounded-3xl border border-slate-200 bg-white shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between gap-5 border-b border-slate-100 px-5 py-5 md:px-6">
              <div><p className="text-xs font-extrabold tracking-[0.14em] text-blue-700">개인 기록 관리</p><h2 id="history-settings-title" className="mt-1 flex items-center gap-2 text-xl font-extrabold text-slate-950"><Settings className="h-5 w-5 text-blue-600" aria-hidden="true" />설정</h2><p id="history-settings-description" className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">수집 범위, 제외 도메인, 자동 동기화 주기 및 데이터 초기화를 관리합니다.</p></div>
              <button ref={settingsCloseButtonRef} type="button" aria-label="설정 닫기" onClick={() => setSettingsOpen(false)} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-100"><X className="h-4 w-4" aria-hidden="true" /></button>
            </div>
            <div className="max-h-[calc(100svh-10rem)] overflow-y-auto p-5 md:p-6">
              <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,.9fr)]">
                <div className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4"><p className="text-sm font-extrabold text-slate-950">내 기록과 수집 제외 목록</p><p className="mt-1 text-xs leading-5 text-slate-600">URL, 제목, 페이지 메타 설명, 방문 시각, 방문 횟수를 안전하게 관리합니다.</p><button type="button" onClick={exportMyData} disabled={isExporting} className="mt-4 inline-flex min-h-10 items-center gap-1.5 rounded-xl border border-blue-200 bg-white px-3 text-xs font-bold text-blue-700 transition hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-100 disabled:cursor-not-allowed disabled:opacity-55">{isExporting ? <RefreshCw className="h-3.5 w-3.5 animate-spin" aria-hidden="true" /> : <Copy className="h-3.5 w-3.5" aria-hidden="true" />}{isExporting ? '내보내기 준비 중' : '내 기록 내보내기'}</button>
        ...
        <footer className="flex flex-col gap-2 py-8 text-xs text-slate-500 md:flex-row md:items-center md:justify-between"><p>탐색 흔적은 암호화된 계정별 저장소에 안전하게 보관되며, 수집 범위와 보존은 사용자가 직접 관리합니다.</p><p className="flex items-center gap-1.5 font-medium text-emerald-700"><ShieldCheck className="h-4 w-4" />수집 범위·보존·차단 규칙은 언제나 사용자가 결정합니다.</p></footer>
      </div>
    </main>
  );
}
