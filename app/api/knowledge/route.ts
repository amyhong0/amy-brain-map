...
async function callNvidiaVisionModel(imageUrls: string[], prompt: string, totalImages?: number): Promise<string | null> {
...
async function fetchInstagramContent(url: string): Promise<{ title: string; content: string; imageUrls: string[]; totalImages?: number } | null> {
  try {
    ...
    if (shortcode) {
      ...
      for (let i = occurrences.length - 1; i >= 0; i--) {
        ...
        if (snippet.includes('"carousel_media"') || snippet.includes('"display_uri"')) {
          ...
          const countMatch = snippet.match(/"carousel_media_count"\s*:\s*(\d+)/);
          totalImages = countMatch ? parseInt(countMatch[1], 10) : 0;
    ...
    // Extract totalImages from snippet if available
    let totalImages = imageUrls.length;
    if (shortcode) {
      const countMatches = [...html.matchAll(/"carousel_media_count"\s*:\s*(\d+)/g)];
      if (countMatches.length > 0) {
        totalImages = Math.max(imageUrls.length, ...countMatches.map(m => parseInt(m[1], 10)));
      }
    }

    return { title: '', content: content + extraAltContent, imageUrls, totalImages };
  } catch (e) {
...
async function fetchWebContent(url: string): Promise<{ title: string; content: string; keywords: string[]; topic?: string; imageUrls?: string[]; totalImages?: number }> {
  try {
    let instagramData: { content: string; imageUrls: string[]; totalImages?: number } | null = null;
    ...
    if (isInstagramUrl(url)) {
      ...
      if (igData) {
        instagramData = { content: igData.content, imageUrls: igData.imageUrls, totalImages: igData.totalImages };
      }
    }
    ...
    let rawTitle = '';
    let totalImages: number | undefined = undefined;

    if (instagramData) {
      // Use Instagram data
      rawContent = instagramData.content;
      imageUrls = [...instagramData.imageUrls];
      totalImages = instagramData.totalImages;
    ...
    if (imageUrls.length > 0) {
      ...
      try {
        const [titleResult, descResult] = await Promise.all([
          ...
          callNvidiaVisionModel(imageUrls, '이 인스타그램 캐러셀 이미지들에 있는 모든 텍스트를 빠짐없이 한국어로 추출해주세요.', totalImages)
        ]);
...
${instagramData ? '인스타그램 게시글의 경우, 이미지에 제목이 있을 경우 그것을 우선 사용하고, 없다면 본문과 이미지 내용을 바탕으로 적합한 제목을 생성하세요.' : ''}`;
    ...
    if (llmResult) {
      try {
        ...
        return { title, content, keywords, topic, imageUrls, totalImages };
      } catch (parseError) {
    ...
    return {
      title: fallbackClean.title || visionTitleCandidate || fallbackKeywords[0] || '제목 없음',
      content: fallbackClean.content || `URL: ${url}\n\n내용 추출 실패 - 직접 방문하여 확인하세요.`,
