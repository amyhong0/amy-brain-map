...
const STARTER_QUESTIONS = [
  EXAMPLE_QUESTION,
  '최근 일주일 동안 반복해서 본 주제는 뭐야?',
  '내가 자주 함께 살펴본 관심사는 무엇이야?',
];
...
function followUpSuggestions(result: QueryResult) {
  ...
  return [`“${focus}”와 연결된 다른 관심은 뭐야?`, `“${focus}”이(가) 가장 활발했던 시점은 언제야?`, '다음으로 검토하면 좋은 연결은 무엇이야?'];
}
...
function extensionBridgeRequest<T>(type: string, payload: Record<string, unknown> = {}): Promise<T> {
  return new Promise((resolve, reject) => {
    const requestId = window.crypto?.randomUUID?.() || `${Date.now()}-${Math.random()}`;
    const timeoutId = window.setTimeout(() => {
      window.removeEventListener('message', handleMessage);
      reject(new Error('Chrome 확장 프로그램 상태를 확인하지 못했습니다. 확장 프로그램이 설치·활성화되어 있는지 확인하세요.'));
    }, 8_000);
    ...
    window.postMessage({ source: 'amy-brain-map-dashboard', type, requestId, ...payload }, window.location.origin);
  });
}

function renderInlineMarkdown(text: string) {
  return text.split(/(\*\*[^*\n]+?\*\*|`[^`\n]+?`)/g).filter(Boolean).map((part, index) => {
    if (part.startsWith('**') && part.endsWith('**')) return <strong key={index} className="font-extrabold text-slate-950">{part.slice(2, -2)}</strong>;
    if (part.startsWith('`') && part.endsWith('`')) return <code key={index} className="rounded bg-slate-200 px-1 py-0.5 font-mono text-[0.82em] font-semibold text-slate-700">{part.slice(1, -1)}</code>;
...
export default function Home() {
  ...
  const [mapHighlightsActive, setMapHighlightsActive] = useState(false);
  const [chatTurns, setChatTurns] = useState<ChatTurn[]>([]);
  const chatLogRef = useRef<HTMLDivElement | null>(null);
  const [selectedCandidate, setSelectedCandidate] = useState<DiscoveryCandidate | null>(null);
  const [selectedNodeDetail, setSelectedNodeDetail] = useState<MapNodeDetail | null>(null);
  const [pendingMapRemoval, setPendingMapRemoval] = useState<MapNodeDetail | null>(null);
  ...
  const refreshAutoSyncState = useCallback(async () => {
    if (!user) {
      setAutoSync(null);
      return;
    }
    ...
    try {
      const result = await extensionBridgeRequest<{ active: boolean; intervalMinutes: number; nextRunAt: number | null }>('auto-sync-status');
      setAutoSync(result);
    } catch {
      setAutoSync(null);
    } finally {
  ...
  useEffect(() => {
    if (settingsOpen) void refreshAutoSyncState();
  }, [settingsOpen, refreshAutoSyncState]);

  const updateAutoSyncInterval = async (intervalMinutes: number) => {
    if (!user || autoSyncLoading) return;
    setAutoSyncLoading(true);
    setAutoSyncNotice('');
    try {
      const result = await extensionBridgeRequest<{ active: boolean; intervalMinutes: number; nextRunAt: number | null }>('set-auto-sync-interval', { intervalMinutes });
      setAutoSync(result);
      setAutoSyncNotice(`자동 수집 간격을 ${AUTO_SYNC_INTERVAL_OPTIONS.find((option) => option.value === intervalMinutes)?.label || `${intervalMinutes}분마다`}로 변경했습니다.`);
    } catch (syncError) {
      setAutoSyncNotice(visitorFacingError(syncError, '자동 수집 간격을 변경하지 못했습니다.'));
    } finally {
      setAutoSyncLoading(false);
    }
  };
  ...
  const highlightedIds = mapHighlightsActive ? queryResult?.highlightedCandidateIds || [] : [];
  const highlightedVisits = mapHighlightsActive ? queryResult?.matchedVisits || [] : [];
  const visibleVisits = queryResult?.matchedVisits || visits;
  const clearMapHighlights = () => {
    setMapHighlightsActive(false);
    setSelectedCandidate(null);
  ...
  const removeNodeFromMap = async () => {
    ...
    const targets = pendingMapRemoval.candidates.filter((candidate) => candidate.status !== 'rejected');
    if (targets.length === 0) {
      setPendingMapRemoval(null);
      clearMapHighlights();
      return;
    }
    ...
    setError('');
    try {
      const updates = await Promise.all(targets.map(async (candidate) => {
        const response = await fetch(`/api/unconscious/candidates/${candidate.id}`, { method: 'PATCH', headers: requestHeaders(), body: JSON.stringify({ status: 'rejected' }) });
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || '지도 항목을 제거하지 못했습니다.');
        return data.candidate as DiscoveryCandidate;
      }));
      const byId = new Map(updates.map((candidate) => [candidate.id, candidate]));
      setCandidates((current) => current.map((candidate) => byId.get(candidate.id) || candidate));
      setPendingMapRemoval(null);
      clearMapHighlights();
      setApprovalNotice(`“${pendingMapRemoval.label}”을(를) 지도에서 제거했습니다. 근거 방문 기록은 그대로 유지됩니다.`);
    } catch (removalError) {
      setError(visitorFacingError(removalError, '지도 항목을 제거하지 못했습니다.'));
    } finally {
  ...
  return (
    <main className="aether-dashboard brain-canvas min-h-screen text-slate-900">
      ...
      <div className="relative w-full px-4 pb-10 md:px-8 lg:px-10">
        ...
        {settingsOpen && <section className="brain-card mt-5 rounded-3xl p-5 md:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><Activity className="h-4 w-4 text-blue-600" aria-hidden="true" /><p className="text-base font-extrabold text-slate-950">자동 기록 수집</p>{autoSyncLoading ? <StatusPill tone="slate">확인 중</StatusPill> : autoSync?.active ? <StatusPill tone="green"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />작동 중</StatusPill> : <StatusPill tone="amber">확장 프로그램 확인 필요</StatusPill>}</div><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">확장 프로그램이 설치·연결된 뒤에는 새로 열어본 페이지를 대기열에 넣고, 설정한 간격마다 자동으로 기록 저장소에 동기화합니다.</p></div>{autoSync?.active && <p className="shrink-0 text-xs leading-5 text-slate-500 sm:text-right">다음 확인 · {autoSync.nextRunAt ? formatDate(autoSync.nextRunAt) : '예약 확인 중'}</p>}</div><div className="mt-5 grid gap-3 rounded-2xl border border-slate-100 bg-slate-50 p-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end"><label className="block text-xs font-bold text-slate-700">자동 수집 간격<select value={autoSync?.intervalMinutes ?? 30} disabled={!autoSync?.active || autoSyncLoading} onChange={(event) => void updateAutoSyncInterval(Number(event.target.value))} className="mt-2 min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-900 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 disabled:cursor-not-allowed disabled:opacity-55">{AUTO_SYNC_INTERVAL_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><button type="button" onClick={() => void refreshAutoSyncState()} disabled={autoSyncLoading} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-blue-200 bg-white px-4 text-xs font-extrabold text-blue-700 transition hover:bg-blue-50 disabled:cursor-not-allowed disabled:opacity-55"><RefreshCw className={`h-3.5 w-3.5 ${autoSyncLoading ? 'animate-spin' : ''}`} aria-hidden="true" />상태 다시 확인</button></div>{autoSyncNotice && <p role="status" aria-live="polite" className="mt-3 text-xs font-semibold leading-5 text-emerald-700">{autoSyncNotice}</p>}{!autoSyncLoading && !autoSync?.active && <p className="mt-3 text-xs leading-5 text-slate-500">상태를 확인할 수 없으면 Chrome 확장 프로그램이 최신 버전으로 설치·활성화되어 있는지 확인한 뒤 이 설정을 다시 여세요.</p>}</section>}
        ...
        <section className="mt-6 grid gap-5 xl:grid-cols-[250px_minmax(0,1fr)_minmax(420px,.82fr)] xl:items-start">
          ...
          <div className="order-1 min-w-0 space-y-5 xl:contents xl:space-y-0"><section className="aether-hero xl:col-span-3 xl:row-start-1 relative isolate overflow-hidden rounded-3xl border p-6 text-white md:p-8"><div aria-hidden="true" className="pointer-events-none absolute -right-24 -top-32 h-72 w-72 rounded-full border border-white/15" /><div aria-hidden="true" className="pointer-events-none absolute -bottom-36 left-[18%] h-64 w-64 rounded-full bg-violet-500/[.14] blur-3xl" /><div className="relative flex flex-col gap-6"><div className="max-w-none"><p className="text-xs font-extrabold tracking-[0.24em] text-blue-200">THOUGHTS, MADE VISIBLE</p><h2 className="display-serif break-keep mt-4 text-[30px] font-medium leading-[1.32] tracking-[0.012em] sm:text-[42px] sm:leading-[1.24] sm:tracking-[0.014em] lg:text-[50px] lg:leading-[1.18] lg:tracking-[0.018em]"><span className="block lg:whitespace-nowrap">스쳐 지나간 웹페이지 속에서,</span><span className="block lg:whitespace-nowrap">내 사고의 흐름을 발견하세요.</span></h2><p className="mt-5 max-w-none text-sm leading-7 text-slate-300 md:text-[15px]">무심코 열어본 페이지들을 모아 어떤 주제를 반복해 살펴봤는지 분석하고, 서로 이어지는 관심의 흐름을 한눈에 보여 줍니다.</p></div><div className="flex shrink-0 flex-col gap-2 lg:self-end lg:items-end"><button type="button" disabled={!user || isHistorySyncing || isAnalyzing} onClick={requestHistoryFromChrome} className="aether-action-primary inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-extrabold transition disabled:cursor-not-allowed disabled:opacity-55">{isHistorySyncing || isAnalyzing ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />}{isHistorySyncing ? 'Chrome 기록을 읽는 중…' : isAnalyzing ? '패턴을 분석하는 중…' : 'Chrome 기록 가져오기'}</button><div className="max-w-[260px] lg:text-right"><p aria-live="polite" className="text-[11px] leading-5 text-slate-300">{historySyncMessage || (user ? 'Chrome에 현재 남아 있는 방문 기록을 읽어와 지도에 반영합니다.' : 'Google 로그인 뒤 본인의 Chrome 프로필을 연결하세요.')}</p>{isHistorySyncing && historySyncProgress !== null && <div className="mt-2 flex items-center gap-2 lg:justify-end"><div role="progressbar" aria-label="Chrome 기록 동기화 진행률" aria-valuemin={0} aria-valuemax={100} aria-valuenow={historySyncProgress} className="h-1.5 w-36 overflow-hidden rounded-full bg-white/15"><div className="h-full rounded-full bg-blue-300 transition-[width] duration-300" style={{ width: `${historySyncProgress}%` }} /></div><span className="min-w-8 text-right text-[11px] font-bold text-blue-200">{historySyncProgress}%</span></div>}</div></div></div></section><div className="xl:col-start-2 xl:row-start-2"><UnconsciousMap candidates={candidates} selectedId={selectedCandidate?.id} highlightedIds={highlightedIds} highlightedVisits={highlightedVisits} onSelect={handleMapNodeSelect} onClearHighlights={clearMapHighlights} />{selectedNodeDetail && <section className="brain-card mt-4 overflow-hidden rounded-3xl border border-blue-100 bg-white shadow-sm" aria-label={`${selectedNodeDetail.label} 상세 정보`}><div className="flex items-start justify-between gap-4 border-b border-slate-100 bg-gradient-to-r from-blue-50 via-white to-violet-50 px-5 py-4"><div className="min-w-0"><p className="text-[11px] font-extrabold tracking-[0.16em] text-blue-700">NODE DETAIL</p><h3 className="mt-1 truncate text-lg font-extrabold text-slate-950">{selectedNodeDetail.label}</h3><p className="mt-1 text-xs leading-5 text-slate-600">이 주제에 연결된 관심과 탐색 근거를 확인합니다.</p></div><button type="button" onClick={clearMapHighlights} aria-label={`${selectedNodeDetail.label} 상세 정보 닫기`} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl text-slate-500 transition hover:bg-white hover:text-slate-900 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-100"><X className="h-4 w-4" aria-hidden="true" /></button></div><div className="grid gap-4 p-5 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]"><div><div className="grid grid-cols-3 gap-2"><div className="rounded-xl border border-slate-100 bg-slate-50 p-2.5"><p className="text-[10px] font-extrabold tracking-[0.1em] text-slate-500">상태</p><p className="mt-1 text-xs font-extrabold text-slate-900">{selectedNodeDetail.candidates.some((candidate) => candidate.status === 'approved' || candidate.status === 'auto_applied') ? '지도 반영됨' : '연결 검토'}</p></div><div className="rounded-xl border border-slate-100 bg-slate-50 p-2.5"><p className="text-[10px] font-extrabold tracking-[0.1em] text-slate-500">연결</p><p className="mt-1 text-xs font-extrabold text-slate-900">{selectedNodeDetail.connections.length}개</p></div><div className="rounded-xl border border-slate-100 bg-slate-50 p-2.5"><p className="text-[10px] font-extrabold tracking-[0.1em] text-slate-500">신뢰도</p><p className="mt-1 text-xs font-extrabold text-slate-900">{Math.round(selectedNodeDetail.confidence * 100)}%</p></div></div><div className="mt-4"><p className="text-xs font-extrabold text-slate-800">연결된 관심</p>{selectedNodeDetail.connections.length ? <div className="mt-2 flex flex-wrap gap-2">{selectedNodeDetail.connections.slice(0, 8).map((connection) => <span key={connection.label} className="rounded-full border border-blue-100 bg-blue-50 px-2.5 py-1 text-[11px] font-semibold text-blue-800">{connection.label} <span className="text-blue-500">{Math.round(connection.score * 100)}%</span></span>)}</div> : <p className="mt-2 text-xs leading-5 text-slate-500">아직 이 주제와 직접 연결된 다른 관심이 없습니다.</p>}</div></div><div className="border-t border-slate-100 pt-4 md:border-l md:border-t-0 md:pl-4 md:pt-0"><p className="text-xs font-extrabold text-slate-800">탐색 근거</p><ul className="mt-2 space-y-2">{selectedNodeDetail.candidates.flatMap((candidate) => candidate.evidence).filter((value, index, values) => values.indexOf(value) === index).slice(0, 3).map((evidence) => <li key={evidence} className="rounded-xl border border-slate-100 bg-slate-50 px-3 py-2 text-xs leading-5 text-slate-600">{evidence}</li>)}</ul><p className="mt-3 text-[11px] leading-5 text-slate-500">근거 도메인 · {[...new Set(selectedNodeDetail.candidates.flatMap((candidate) => candidate.sourceDomains))].slice(0, 5).join(' · ') || '기록 근거 확인 중'}</p><button type="button" onClick={() => setPendingMapRemoval(selectedNodeDetail)} className="mt-4 inline-flex min-h-10 items-center gap-1.5 rounded-xl border border-rose-200 bg-white px-3.5 text-xs font-bold text-rose-700 transition hover:bg-rose-50 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-rose-100"><Trash2 className="h-3.5 w-3.5" aria-hidden="true" />지도에서 제거</button><p className="mt-2 text-[11px] leading-5 text-slate-500">근거 방문 기록은 삭제하지 않습니다.</p></div></div></section>}</div></div>
          ...
          <aside className="order-3 xl:col-start-3 xl:row-start-2 xl:sticky xl:top-24">
            <section className="brain-card flex h-[min(680px,calc(100svh-2rem))] flex-col overflow-hidden rounded-3xl lg:h-[min(720px,calc(100svh-7rem))]">
              ...
              <form onSubmit={ask} className="aether-chat-composer border-t p-4">
                <label className="sr-only" htmlFor="map-question">브레인 맵에 질문하기</label>
                <textarea id="map-question" value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={(event) => { if (event.key !== 'Enter' || event.shiftKey || event.nativeEvent.isComposing) return; event.preventDefault(); void ask(); }} disabled={!user || isLoading} rows={3} aria-describedby="map-question-help" className="aether-chat-input w-full resize-none rounded-2xl border px-3.5 py-3 text-sm leading-6 outline-none disabled:cursor-not-allowed" />
                <div className="mt-3 flex items-center justify-between gap-2"><div><label className="aether-chat-search-label inline-flex min-h-11 cursor-pointer items-center gap-2.5 text-xs font-bold"><span>웹 검색</span><span className={`aether-chat-toggle relative h-7 w-12 rounded-full ${webSearchEnabled ? 'aether-chat-toggle-on' : ''} ${!user || isLoading ? 'opacity-50' : ''}`}><input checked={webSearchEnabled} onChange={(event) => setWebSearchEnabled(event.target.checked)} disabled={!user || isLoading} type="checkbox" role="switch" aria-label="웹 검색" className="peer sr-only" /><span aria-hidden="true" className={`aether-chat-toggle-thumb absolute top-1 h-5 w-5 rounded-full ${webSearchEnabled ? 'translate-x-6' : 'translate-x-1'}`} /></span></label><p id="map-question-help" className="mt-0.5 text-[10px] font-medium text-slate-500">Enter로 전송 · Shift + Enter로 줄바꿈</p></div><button type="submit" disabled={!user || isLoading || !question.trim()} className="aether-chat-send inline-flex min-h-10 items-center gap-2 rounded-xl px-3.5 text-xs font-bold transition disabled:cursor-not-allowed disabled:opacity-40">{isLoading ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}{isLoading ? '찾는 중' : '보내기'}</button></div>
              </form>
            </section>
          </aside>
        </section>
