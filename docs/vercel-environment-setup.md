...
네. 이 서비스는 **여러 사용자가 각자의 Google 계정으로 로그인하고, 각자의 Chrome 방문 기록만 별도로 저장·분석하는 구조**로 구현되어 있습니다. 운영자만 Vercel·PostgreSQL·GCS의 공용 인프라 비밀값을 등록하며, 일반 사용자는 Google 로그인 후 Chrome 기록 가져오기 버튼을 누르기만 하면 됩니다.
...
| **Chrome 연결** | 로그인된 대시보드가 내부 단기 권한을 해당 Chrome 프로필의 설치 토큰으로 자동 교환합니다. 사용자는 코드를 입력하지 않습니다. |
...
7. Google 계정이 서로 다른 두 브라우저 프로필로 로그인해, 각 계정에서 Chrome 기록 가져오기 버튼만으로 자신의 Chrome 프로필이 자동 연결되고 기록이 분리되는지 확인합니다.
