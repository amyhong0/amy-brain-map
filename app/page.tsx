          ...
          <aside className="order-3 xl:col-start-3 xl:row-start-2 xl:sticky xl:top-24">
            <section className="brain-card flex h-[min(680px,calc(100svh-2rem))] flex-col overflow-hidden rounded-3xl lg:h-[min(720px,calc(100svh-7rem))]">
              ...
              <form onSubmit={ask} className="aether-chat-composer border-t p-4">
                ...
                <textarea id="map-question" value={question} onChange={(event) => setQuestion(event.target.value)} disabled={!user || isLoading} rows={3} placeholder={EXAMPLE_QUESTION} className="aether-chat-input w-full resize-none rounded-2xl border px-3.5 py-3 text-sm leading-6 outline-none disabled:cursor-not-allowed" />
                <div className="mt-3 flex items-center justify-between gap-2"><label className="aether-chat-search-label inline-flex min-h-11 cursor-pointer items-center gap-2.5 text-xs font-bold"><span>웹 검색</span><span className={`aether-chat-toggle relative h-7 w-12 rounded-full ${webSearchEnabled ? 'aether-chat-toggle-on' : ''} ${!user || isLoading ? 'opacity-50' : ''}`}><input checked={webSearchEnabled} onChange={(event) => setWebSearchEnabled(event.target.checked)} disabled={!user || isLoading} type="checkbox" role="switch" aria-label="웹 검색" className="peer sr-only" /><span aria-hidden="true" className={`aether-chat-toggle-thumb absolute top-1 h-5 w-5 rounded-full ${webSearchEnabled ? 'translate-x-6' : 'translate-x-1'}`} /></span></label><button disabled={!user || isLoading || !question.trim()} className="aether-chat-send inline-flex min-h-10 items-center gap-2 rounded-xl px-3.5 text-xs font-bold transition disabled:cursor-not-allowed disabled:opacity-40">{isLoading ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}{isLoading ? '찾는 중' : '보내기'}</button></div>
              </form>
            </section>
          </aside>
        </section>
