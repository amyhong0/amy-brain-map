  ...
  return (
    <main className="brain-canvas min-h-screen text-slate-900">
      ...
      <div className="relative mx-auto max-w-[1680px] px-4 pb-10 md:px-8 lg:px-10">
        ...
        {!token && <section className="brain-card mt-6 rounded-3xl border-amber-200 bg-gradient-to-r from-amber-50 via-white to-blue-50 p-5 md:p-6"><div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div className="max-w-2xl"><div className="flex items-center gap-2 text-sm font-bold text-amber-900"><LockKeyhole className="h-4 w-4" />먼저 나의 기록을 연결하세요</div><p className="mt-2 text-sm leading-6 text-slate-600">여기에 입력할 값은 <code className="rounded bg-amber-100 px-1.5 py-0.5 font-mono text-xs font-semibold text-amber-900">BROWSER_HISTORY_INGEST_TOKEN</code> 환경 변수입니다. 개발 환경에서는 프로젝트 루트의 <code className="rounded bg-amber-100 px-1.5 py-0.5 font-mono text-xs font-semibold text-amber-900">.env.local</code>에서 설정하고, Vercel 배포에서는 <strong>Project Settings → Environment Variables</strong>에서 확인하거나 새 값으로 다시 설정하세요. 같은 값을 Chrome 확장 프로그램의 <strong>연결·개인정보 설정 → 개인 보호 키</strong>에도 입력해야 합니다.</p></div><div className="flex w-full max-w-md gap-2"><div className="relative flex-1"><label className="sr-only" htmlFor="history-token">BROWSER_HISTORY_INGEST_TOKEN</label><input id="history-token" value={tokenDraft} onChange={(event) => setTokenDraft(event.target.value)} type={showToken ? 'text' : 'password'} placeholder="BROWSER_HISTORY_INGEST_TOKEN 값" className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 pr-10 text-sm text-slate-900 shadow-sm outline-none placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100" /><button type="button" aria-label={showToken ? '보호 키 숨기기' : '보호 키 표시'} onClick={() => setShowToken((shown) => !shown)} className="absolute inset-y-0 right-2 grid place-items-center rounded-lg px-1.5 text-slate-500 hover:text-blue-700">{showToken ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div><button type="button" onClick={saveToken} className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-bold text-white shadow-lg shadow-blue-500/20 transition hover:bg-blue-700">연결</button></div></div></section>}
