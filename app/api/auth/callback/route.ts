...
function failureRedirect(code: string) {
  return NextResponse.redirect(new URL(`/?authError=${encodeURIComponent(code)}`, appOrigin()));
}
...
export async function GET(request: NextRequest) {
  ...
  if (!code || !state || !savedState || !verifyOAuthState(state) || !verifyOAuthState(savedState) || state !== savedState) {
    const response = failureRedirect('state');
  ...
  if (!clientId || !clientSecret) {
    const response = failureRedirect('configuration');
    clearOAuthStateCookie(response);
    return response;
  }
  ...
  try {
    ...
    const response = failureRedirect('oauth');
    clearOAuthStateCookie(response);
    return response;
  }
}
