...
const STATUS_STYLE: Record<CandidateStatus, { label: string; color: string }> = {
  pending: { label: '연결 검토 대상', color: '#f59e0b' },
...
const EDGE_STYLE: Record<EdgeKind, { label: string; color: string; dash?: string }> = {
  journey: { label: '같은 탐색 흐름', color: '#2563eb' },
  cooccurrence: { label: '같은 페이지에서 함께 나타남', color: '#7c3aed', dash: '4 5' },
  'shared-domain': { label: '같은 도메인에서 반복 탐색', color: '#94a3b8', dash: '2 7' },
};
  ...
  return (
    <section className="brain-card overflow-hidden rounded-3xl">
      <div className="flex flex-col gap-4 border-b border-slate-100 px-5 py-5 md:flex-row md:items-center md:justify-between md:px-6">
        <div className="flex gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-violet-50 text-violet-700"><Network className="h-5 w-5" aria-hidden="true" /></div><div><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-violet-600">Your thought landscape</p><h2 className="mt-1 text-lg font-extrabold text-slate-950">반복 관심과 연결의 지도</h2><p className="mt-1 text-xs leading-5 text-slate-500">실선은 같은 탐색 흐름, 점선은 같은 페이지 또는 도메인에서 함께 나타난 관심입니다.</p></div></div>
        <div className="inline-flex rounded-xl border border-slate-200 bg-slate-50 p-1" aria-label="지도 보기 범위">{([['all', '전체'], ['pending', '연결 검토'], ['confirmed', '지도에 반영됨']] as const).map(([key, label]) => <button type="button" key={key} onClick={() => setView(key)} aria-pressed={view === key} className={`min-h-9 rounded-lg px-3 text-xs font-bold transition ${view === key ? 'bg-white text-blue-700 shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:text-slate-900'}`}>{label}</button>)}</div>
      </div>
