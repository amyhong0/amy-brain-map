  ...
  return (
    <main className="brain-canvas min-h-screen text-slate-900">
      ...
      <div className="relative mx-auto max-w-[1680px] px-4 pb-10 md:px-8 lg:px-10">
        ...
        {!authLoading && !user && <section className="brain-card mt-6 rounded-3xl border-amber-200 bg-gradient-to-r from-amber-50 via-white to-blue-50 p-5 md:p-6"><div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div className="max-w-2xl"><div className="flex items-center gap-2 text-sm font-bold text-amber-900"><UserRound className="h-4 w-4" />먼저 Google 계정으로 나의 지도를 만드세요</div><p className="mt-2 text-sm leading-6 text-slate-600">로그인하면 방문 기록, 관심 축, 차단 규칙이 Google 계정별로 분리됩니다. 로그인 뒤 대시보드에서 발급하는 일회용 연결 코드로 본인의 Chrome 프로필만 연결할 수 있습니다.</p></div><button type="button" onClick={startGoogleLogin} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-blue-500/20 transition hover:bg-blue-700"><LogIn className="h-4 w-4" />Google로 시작하기</button></div></section>}
